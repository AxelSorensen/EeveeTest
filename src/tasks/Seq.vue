    ...
    <div class="bg-purple-500 flex justify-between p-2 rounded-t-md">
      <div class="flex gap-2 flex-wrap">
        <div class="rounded-md p-2 px-6 border border-white border-1 font-bold text-center relative select-none"
          ...
          v-for="label, index in tasks[selectedTaskId.value]?.labels" @click="this.selectedLabelId.value = index">
      ...
      <div class="items-center justify-center flex gap-2">
        <div class="group cursor-pointer " @click="keyboardMode = true">
          <font-awesome-icon class="mr-2 text-lg"
            :class="keyboardMode ? 'text-purple-200' : 'group-hover:text-purple-400 text-purple-800'" icon="fa-solid fa-keyboard" />
        </div>
        <div class="group cursor-pointer" @click="keyboardMode = false">
          <font-awesome-icon class="mr-2 text-lg"
            :class="!keyboardMode ? 'text-purple-200' : 'group-hover:text-purple-400 text-purple-800'" icon="fa-solid fa-mouse" />
        </div>
      </div>
    ...
    <div class="justify-center items-center flex bg-white rounded-b-md">
      <div v-if="data.length > 0" class="flex justify-center items-end  rounded-b-md gap-1 flex-wrap gap-y-4 m-8">
        <span v-for="word, index in data[currentSentenceId.value]?.words"
          ...
          :class="tasks[selectedTaskId.value]?.labels.includes(word[tasks[selectedTaskId.value].output_index]) ? 'bg-purple-500 text-white hover' : 'hover:bg-gray-300', index == selectedWordId.value && !tasks[selectedTaskId.value]?.labels.includes(word[tasks[selectedTaskId.value].output_index]) && keyboardMode ? 'bg-gray-300' : '', index == selectedWordId.value && tasks[selectedTaskId.value]?.labels.includes(word[tasks[selectedTaskId.value].output_index]) && keyboardMode ? 'bg-purple-400' : ''"
...
export default {
  ...
  methods: {
    setLabel(word_index, currentSentenceId, label) {
      if (this.data[currentSentenceId.value].words[word_index][this.tasks[this.selectedTaskId.value].output_index] != '_') {
